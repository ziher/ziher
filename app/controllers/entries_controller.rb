class EntriesController < ApplicationController
  ...
  def new
    @journal = Journal.includes(:journal_type, :unit).find(params[:journal_id])
  ...
  # GET /entries/1/edit
  def edit
    @entry = Entry.includes(:items, journal: [:journal_type, :unit]).find(params[:id])
    authorize! :update, @entry
    @journal = @entry.journal
    @other_journals = @journal.journals_for_linked_entry
    @categories = Category.where(:year => @entry.journal.year, :is_expense => @entry.is_expense).order(:position)
    create_empty_items(@entry, @journal.year)
